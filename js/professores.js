...
const dataNascimentoProfessor =
  document.getElementById(
    "dataNascimentoProfessor"
  );

const linkEventosProfessor =
  document.getElementById(
    "linkEventosProfessor"
  );

const linkZoomReposicaoProfessor =
  document.getElementById(
    "linkZoomReposicaoProfessor"
  );

const listaMaterias =
  document.getElementById("listaMaterias");

const btnSalvarProfessor =
  document.getElementById(
    "btnSalvarProfessor"
  );
...
function criarLinhaMateria(materia) {
  ...
  linhaTopo.style.alignItems =
    "center";

  linhaTopo.style.justifyContent =
    "space-between";
  ...
  textoValor.style.fontSize =
    "13px";

  textoValor.style.opacity =
    "0.85";
  ...
  inputValor.className =
    "input-valor-materia";

  inputValor.dataset.materiaId =
    materia.id;

  inputValor.disabled = true;

  inputValor.style.width =
    "140px";

  inputValor.style.padding =
    "8px";

  inputValor.style.borderRadius =
    "8px";
...
function renderMaterias() {
  ...
  materiasCache.forEach(
    (materia) => {
      listaMaterias.appendChild(
        criarLinhaMateria(materia)
      );
    }
  );
}
...
form.addEventListener(
  ...
  async (e) => {
    ...
    const data_nascimento =
      dataNascimentoProfessor.value ||
      null;

    const link_eventos =
      linkEventosProfessor.value
        .trim() ||
      null;

    const link_zoom_reposicao =
      linkZoomReposicaoProfessor.value
        .trim() ||
      null;
    ...
    if (
      ...
      mostrarMensagem(
        "Selecione pelo menos 1 matéria.",
        false
      );
    ...
    /* =====================
       DESATIVAR BOTÃO
    ===================== */
    btnSalvarProfessor.disabled = true;

    btnSalvarProfessor.textContent =
      "Salvando professor...";
    ...
    try {
      ...
      /* =====================
         VÍNCULOS COM MATÉRIAS
      ===================== */
      const payloadVinculos =
      ...
      if (errVinc) {
        console.error(
          "Erro ao vincular matérias:",
          errVinc
        );

        mostrarMensagem(
          "Professor salvo, mas ocorreu um erro ao vincular as matérias.",
          false
        );
      ...
      console.error(
        "Erro inesperado ao cadastrar professor:",
        erro
      );

      mostrarMensagem(
        "Ocorreu um erro inesperado ao cadastrar o professor.",
        false
      );

    } finally {
      btnSalvarProfessor.disabled =
        false;
