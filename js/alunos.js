...
async function carregarBases() {
  ...
  const { data: vinculos } = await supabase
    ...
    .select(`materia_id, professor:professor_id ( id, nome )`);
...
function adicionarCurso() {
  ...
  materiasCache.forEach((m) => {
    selMateria.appendChild(criarOption(m.id, m.nome));
  });
  ...
  selMateria.addEventListener("change", async () => {
    ...
    modulos.forEach((m) => {
      selModulo.appendChild(criarOption(m.id, m.nome));
    });

    const professores = professoresPorMateria[materiaId] || [];
  ...
  wrapper.querySelector(".remover").onclick = () => wrapper.remove();
}
...
form.addEventListener("submit", async (e) => {
  ...
  // ✅ AGORA só nome é obrigatório
  if (!nome) {
    mostrarMensagem("⚠️ Preencha o nome.", false);
    return;
  }
