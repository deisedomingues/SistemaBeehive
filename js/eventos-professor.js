...
function filtrarPorPeriodo(lista) {
  const periodo = filtroPeriodoEventos?.value || "todos";
...
function renderListaResponsavel() {
  ...
  if (!listaFiltrada.length) {
    listaEventosResponsavel.innerHTML = `
      <p class="vazio-box">
        Nenhum evento responsável encontrado neste filtro.
        Tente selecionar "Todos os eventos" ou "Eventos anteriores".
      </p>
    `;
    return;
  }
...
function renderListaApoio() {
  ...
  if (!listaFiltrada.length) {
    listaEventosApoio.innerHTML = `
      <p class="vazio-box">
        Nenhum evento de apoio encontrado neste filtro.
        Tente selecionar "Todos os eventos" ou "Eventos anteriores".
      </p>
    `;
    return;
...
async function carregarEventosComoResponsavel() {
  ...
  if (error) {
    ...
    mostrarMensagem(
      "Não foi possível carregar os eventos como responsável.",
      "erro"
    );
